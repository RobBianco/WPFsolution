    ...
    public sealed class VirtualFileDataObject : System.Runtime.InteropServices.ComTypes.IDataObject, IAsyncOperation
    {
        ...
        IEnumFORMATETC System.Runtime.InteropServices.ComTypes.IDataObject.EnumFormatEtc(DATADIR direction)
        {
            if (direction == DATADIR.DATADIR_GET)
            {
                ...
                if (_dataObjects != null && NativeMethods.SUCCEEDED(NativeMethods.SHCreateStdEnumFmtEtc((uint)(_dataObjects.Count), _dataObjects.Select(d => d.FORMATETC).ToArray(), out var enumerator)))
                {
                    return enumerator;
                }
            ...
            if (NativeMethods.SUCCEEDED(hr))
            {
                ...
                if (_dataObjects != null)
                {
                    var dataObject = _dataObjects
                        .FirstOrDefault(d => (d.FORMATETC.cfFormat == formatCopy.cfFormat) &&
        ...
        int System.Runtime.InteropServices.ComTypes.IDataObject.QueryGetData(ref FORMATETC format)
        {
            ...
            if (formatMatches == null) return NativeMethods.S_OK;
            ...
            var aspectMatches = tymedMatches.Where(d => d.FORMATETC.dwAspect == formatCopy.dwAspect).ToList();
        ...
        private DragDropEffects? GetDropEffect(short format)
        {
            ...
            var dataObject = _dataObjects?.LastOrDefault(d => (format == d.FORMATETC.cfFormat) &&
                                                              d.FORMATETC is { dwAspect: DVASPECT.DVASPECT_CONTENT, tymed: TYMED.TYMED_HGLOBAL });
        
            if (dataObject is not { GetData: not null }) return null;
        
            // Read the value and return it
            var result = dataObject.GetData();
        
            if (!NativeMethods.SUCCEEDED(result.Item2)) return null;
        ...
        public static DragDropEffects DoDragDrop(DepenedecyObject dragSource, System.Runtime.InteropServices.ComTypes.IDataObject dataObject, DragDropEffects allowedEffects)
        {
            var finalEffect = new int[1];
            try
            {
            ...
            {
                if ((dataObject is VirtualFileDataObject { IsAsynchronous: false, _inOperation: true } virtualFileDataObject))
                {
                    // Call the end action and exit the operation
